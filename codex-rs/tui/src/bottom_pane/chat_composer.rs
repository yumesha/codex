            ...
            KeyEvent {
                ...
                let in_slash_context = matches!(self.active_popup, ActivePopup::Command(_))
                    || self
                        ...
                        .unwrap_or("")
                        .starts_with('/');
                // When paste-burst detection is disabled, Enter handling must behave like normal
                // typing: no buffering and no "Enter becomes newline" suppression window.
                //
                // That keeps slash-command dispatch (and normal submission) predictable.
                ...
                {
                    // Only paste-burst mode uses the Enter suppression window. Outside of that,
                    // Enter must submit normally so we don't surprise the user after a paste.
                    self.textarea.insert_str("\n");
